<template>
    <div class="reon-mailer-users-editor">
        ...
        <div class="reon-mailer-users-editor__btns">
            <div
                v-if="(emailValidate === statuses.noAccess || emailValidate === statuses.wrongData)" 
                class="reon-mailer-users-editor__message">
                    <span v-if="emailValidate === statuses.noAccess">Почта с таким email'ом уже прикреплена к этому аккаунту</span>
                    <span v-if="emailValidate === statuses.wrongData">Введены неверные данные (<a target="_blank" href="https://drive.google.com/file/d/1f4USZYwKFBIluzYSvvPUtjnbTNHGWa0g/view?usp=sharing">здесь</a>)</span>
            </div>
            <n-button
                type="primary"
...
export default {
    ...
    methods: {
        ...
        async saveEmail() {
            ...
            const emailVerifyStatus = await this.$store.dispatch('users/verifyEmail', { email: this.emailChanges, userId: this.userId, title: this.userTitle })
    ...
    watch: {
        'emailChanges.login': function() {
            this.emailDuplicate = this.statuses.beEdited
        }
    },
