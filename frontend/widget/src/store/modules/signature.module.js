...
export default {
    ...
    actions: {
        async loadSignatures({ state, commit }, managers) {
            ...
            const { url } = state
            const { data: loadedSignatures } = await axiosMailer.post(`${url.getSignatures}?role=${isAdmin ? roles.admin : roles.user}&subdomain=${subdomain}&userId=${userId}`)
        ...
        async saveSignature({ state, commit }, newSignature) {
            const { status } = await axiosMailer.post(state.url.addSignature, {
                ...newSignature
            })
