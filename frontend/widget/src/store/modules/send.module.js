...
const { roles, entityCards } = consts
...
export default {
    ...
    actions: {
        ...
        async loadLinkedLeads() {
            if(APP.getWidgetsArea() === entityCards.lead) {
                const leadId = APP.data.current_card.id
                const leadName = APP.data.current_card.model.defaults['lead[NAME]']
                return [{ id: leadId, name: leadName }]
            }
            if(APP.getWidgetsArea() === entityCards.contact) {
                const contactId = APP.data.current_card.id
                const { data: contactData } = await axios.get(`/api/v4/contacts?id=${contactId}&with=leads`)
            ...
            if(APP.getWidgetsArea() === entityCards.company) {
                const companyId = APP.data.current_card.id
                const { data: companyData } = await axios.get(`/api/v4/companies?id=${companyId}&with=leads`)
                const [ company ] = companyData._embedded.companies
                const { leads: ids } = company._embedded
                if(ids.length) {
                    const { data: leadsData  } = await axios.get(`/api/v4/leads?${ids.map(({id}, index) => `filter[id][${index}]=${id}`).join("&")}`)
                    const { leads: linkedLeads } = leadsData._embedded
                    return linkedLeads
