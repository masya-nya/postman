...
export default {
    ...
    actions: {
        async getPatterns({ state, commit }) {
            ...
            const { data: fetchPatterns } = await axiosMailer.post(`${state.url.getPatterns}?subdomain=${subdomain}&id=${id}&role=${isAdmin ? consts.roles.admin : consts.roles.user}`)
        ...
        async getPattern({ state, commit }) {
            ...
            const { currentPattern, url } = state
            if(currentPattern === '') {
        ...
        async postPatternChanges({ state, commit }, pattern) {
            ...
            let patternObjCopy = Object.assign({}, pattern);
            ...
            await axiosMailer.post(url.postPattern, 
                formData, 
                {
                    headers: {
        ...
        async deletePattern({ state, commit }) {
            ...
            await axiosMailer.post(url.postDeletePattern + subdomain + '&id=' + currentPattern)
